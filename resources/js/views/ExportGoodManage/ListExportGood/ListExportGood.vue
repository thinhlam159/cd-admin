<template>
  <div class="p-5 mt-8 mx-5 bg-white">
    <div class="w-full h-8 flex justify-between">
      <div class="ml-2">
        <ButtonAddNew @clickBtn="goToAdd" :text="addNewExportGood"/>
      </div>
    </div>
...
<!--          </th>-->
        ...
        <tr>
          ...
          <th class="border py-1">Xuất kho</th>
        </tr>
        </thead>
        <tbody class="[&>*:nth-child(odd)]:bg-[#f9f9f9]">
        <template v-for="(item, index) in listExportGood">
          <tr v-if="item.export_good_products.length === 0">
...
<!--                <ButtonEdit @clickBtn="() => goToDetail(item.export_good_id)" text="Cập nhật"/>-->
<!--              </div>-->
<!--            </td>-->
          </tr>
          <tr v-else v-for="(subItem, subIndex) in item.exportGoodProducts"
              :key="subItem.product_attribute_value_id">
            <td v-if="subIndex === 0" :rowspan="item.export_good_products.length" class="border text-center">
...
import {getListExportGoodFromApi} from "@/api";
...
const addNewExportGood = "Tạo đơn xuất kho";
const listExportGood = ref([]);
...
const goToAdd = () => {
  router.push(`${ROUTER_PATH.EXPORT_GOOD_MANAGE}/${ROUTER_PATH.ADD}`);
}

const goToDetail = (id) => {
  router.push(`${ROUTER_PATH.EXPORT_GOOD_MANAGE}/${ROUTER_PATH.DETAIL}/${id}`);
}

const getListExportGood = async (param) => {
  try {
    const res = await getListExportGoodFromApi(param)
    listExportGood.value = res.data.map((item) => {
      return {
        ...item,
        date: item.export_good_date,
        exportGoodProducts: item.export_good_products.map( product =>  {
          return {
            ...product,
...
const handleBackPage = (page) => {
  getListExportGood({page})
}
const handleNextPage = (page) => {
  getListExportGood({page})
}

getListExportGood({page: pageCurrent.value});

store.state[MODULE_STORE.COMMON.NAME].breadcrumbCurrent = 'Xuất hàng tạo thành phẩm'
